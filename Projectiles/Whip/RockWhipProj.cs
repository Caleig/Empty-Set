...
/// 岩流鞭弹幕
/// </summary>
public class RockWhipProj : ModProjectile
{
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Vector2 origin = new Vector2((rectangle.Width / 2f), 2f);
        Vector2 value2 = list[0];
        for (int i = 0; i < list.Count - 1; i++)
        {
            if (i == 0)
            {
                value = ModContent.Request<Texture2D>("EmptySet/Projectiles/Whip/RockWhipProj_handle").Value;
            }
            else if (i == list.Count - 2)
            {
                value = ModContent.Request<Texture2D>("EmptySet/Projectiles/Whip/RockWhipProj_edge").Value;
            }
            else
            {
                value = ModContent.Request<Texture2D>("EmptySet/Projectiles/Whip/RockWhipProj_section").Value;
            }
            ...
            Main.spriteBatch.Draw(value, value2 - Main.screenPosition, null, color, rotation, origin, 1,
                SpriteEffects.None, 0f);
            value2 += vector2;
            if (Vector2.Distance(Main.player[Projectile.owner].Center, Projectile.Center) > 80 &&
                Vector2.Distance(Main.player[Projectile.owner].Center, Projectile.Center) < 200 &&
                Main.rand.NextBool(8))
                Dust.NewDustDirect(list[^3], 1, 1, DustID.BlueTorch, vector2.X, vector2.Y,0,default,3f).noGravity = true;
