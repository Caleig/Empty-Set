...
{
    internal class CloudStarEyeProj:ModProjectile
    {
		...
		public sealed override void SetDefaults()
		{
			...
			// 下面的这些是召唤武器需要的
			Projectile.friendly = false; // 只控制它是否在接触时对敌人造成伤害
		...
		public override void AI()
		{
			...
			Movement(foundTarget, distanceFromTarget, targetCenter, distanceToIdlePosition, vectorToIdlePosition,owner);
			Visuals();
		}
		...
		private bool CheckActive(Player owner)
		{
			...
			{
				owner.ClearBuff(ModContent.BuffType<CloudStarEyeBuff>());

				return false;
			}

			if (owner.HasBuff(ModContent.BuffType<CloudStarEyeBuff>()))
			{
		...
		bool init = false;
		int laserTimer = 0;
		private void Movement(bool foundTarget, float distanceFromTarget, Vector2 targetCenter, float distanceToIdlePosition, Vector2 vectorToIdlePosition, Player owner)
		{
			// 默认移动参数(这里用于攻击)
			float speed = 8f;
			float inertia = 20f;
			...
			if (foundTarget)
			{
				if(distanceFromTarget> 200) init = false;
				// 有一个目标:攻击(这里是飞向敌人)
				if (distanceFromTarget > d && !init) 
				{
				...
				{
					laserTimer++;
					if (laserTimer >= attackFrequency) 
					{
						Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.Normalize(targetCenter - Projectile.Center) * 10, ModContent.ProjectileType<CloudStarEyeLaserProj>(), Projectile.originalDamage, Projectile.knockBack, owner.whoAmI);
						SoundEngine.PlaySound(SoundID.Item33, Projectile.position);
						laserTimer = 0;
					}
					Projectile.velocity = Vector2.Zero;
					Projectile.Center = Vector2.Normalize(Projectile.Center - targetCenter).RotatedBy(MathHelper.ToRadians(1.5f))*160 + targetCenter;
