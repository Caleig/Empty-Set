...
{
    internal class LavaHunterProj:ModProjectile
    {
		...
		public sealed override void SetDefaults()
		{
			...
			Projectile.minionSlots = 0; // 这个仆从从玩家可用的仆从槽总数中占有的槽数(稍后详细介绍)
		...
		private void spawn()
		{
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				...
				{
					int type = i == SEGMENT_NUMBER - 1 ? ModContent.ProjectileType<LavaHunterTailProj>() : ModContent.ProjectileType<LavaHunterBodyProj>();
					int n = Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.Zero, type, Projectile.damage, Projectile.knockBack, owner.whoAmI);

					if (n != Main.maxProjectiles)
					{
						Main.projectile[n].ai[0] = prev;
						Main.projectile[n].ai[1] = Projectile.whoAmI;
						Main.projectile[n].originalDamage = Projectile.originalDamage;
		...
		{
			Vector2 idlePosition = owner.Center + Main.rand.NextVector2Circular(300, 300);
		...
		private void Movement(bool foundTarget, float distanceFromTarget, Vector2 targetCenter, float distanceToIdlePosition, Vector2 vectorToIdlePosition)
		{
			...
			else
			{
				...
				if (distanceToIdlePosition > 40f)
				{
					...
					{
						Projectile.velocity =(Projectile.velocity * inertia + vectorToIdlePosition) / inertia;
					}
				}
