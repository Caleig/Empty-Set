...
/// 运动轨迹待修改
/// </summary>
public class VirtualJungleSickle : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Projectile.Center.Distance(new Vector2(Projectile.localAI[0], Projectile.localAI[1])) < 10 || timer >= 180)
            Projectile.velocity = Vector2.Zero;
        if (timer >= 140) Projectile.alpha += 5;
        var dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.GreenTorch);
        dust.noGravity = true;
    }
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        for (int i = 1; i < Len; i++)
        {
            ...
            Vector2 pos = (Projectile.oldPos[i] + new Vector2(Projectile.spriteDirection == 1 ? 42 : 12, 6) - oldCenter).RotatedBy(Projectile.oldRot[i]) + oldCenter;
            list.Add(new(
