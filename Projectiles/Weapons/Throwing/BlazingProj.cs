...
/// 耀炎弹幕
/// </summary>
public class BlazingProj : ModProjectile
{
    ...
    {
        Projectile.width = 80; //已精确测量
        Projectile.height = 80;
        Projectile.friendly = true;
        Projectile.penetrate = 9 + 1;
        Projectile.timeLeft = 20 * EmptySet.Frame;
	...
	{
        var yellow = Dust.NewDustDirect(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.OrangeTorch, Scale: Main.rand.NextFloat(2f, 3f));
        yellow.noGravity = true;
        var blue = Dust.NewDustDirect(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.RedTorch, Scale: Main.rand.NextFloat(1f, 2f));
        blue.noGravity = true;
		...
		if (Projectile.ai[0] == 0f)
		{
			...
			if (Projectile.ai[1] >= 8f || Projectile.penetrate == 1)
			{
				Projectile.ai[0] = 1f;
				Projectile.ai[1] = 0f;
	    ...
	    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        target.AddBuff(BuffID.OnFire3, 7 * EmptySet.Frame);
        if (hit.Crit)
        {
            Projectile projectile = Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), Projectile.position, Vector2.Zero,
                ModContent.ProjectileType<FlameBallExplode>(), (int)Projectile.damage, 1f, Projectile.owner);
    ...
    public class FlameBallExplode : ModProjectile
    {
    ...
    public override void AI()
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            num1 *= num3;
            num2 *= num3;
            Dust dust = Dust.NewDustDirect(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, DustID.OrangeTorch, Scale: Main.rand.NextFloat(3f, 4f));
            dust.noGravity = true;
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        => target.AddBuff(BuffID.Daybreak, 5 * EmptySet.Frame);
    }
}
